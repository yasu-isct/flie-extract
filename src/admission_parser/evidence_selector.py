...
def build_evidence_selector(profile: ApplicantProfileV2) -> EvidenceSelector:
...
def score_chunk_for_evidence(chunk: TextChunk, selector: EvidenceSelector) -> dict[str, Any]:
    text = chunk_text(chunk)
    score = 0
    reasons: list[str] = []

    if selector.section_anchors and contains_any(text, selector.section_anchors):
        score += 7
        reasons.append("matched_section_anchor")
    if selector.english_aliases and contains_any(text, selector.english_aliases):
        score += 5
        reasons.append("matched_english_test")
    if (
        selector.background_aliases
        and contains_any(text, selector.background_aliases)
        and contains_any(text, ACTION_KEYWORDS)
    ):
        score += 4
        reasons.append("matched_background")
    if selector.degree_aliases and contains_any(text, selector.degree_aliases):
        score += 3
        reasons.append("matched_degree_level")
    if selector.exam_type_aliases and contains_any(text, selector.exam_type_aliases):
        score += 3
        reasons.append("matched_exam_type")
    if selector.include_global_sections and contains_any(text, selector.global_section_keywords):
        score += 3
        reasons.append("matched_global_section")

    negative_hit = contains_any(text, selector.negative_keywords)
    if negative_hit:
        score -= 3
    ...
    other_program_only = (
        ...
        and bool(selector.section_anchors)
        and not contains_any(text, selector.section_anchors)
    ...
    keep = score >= (4 if selector.strict_mode else 2) and not other_program_only
    return {
        "score": score,
        "keep": keep,
        "reasons": reasons,
...
def select_chunks_by_evidence_selector(
    ...
    for index, chunk in enumerate(chunks):
        decision = score_chunk_for_evidence(chunk, selector)
        decision["index"] = index
        decisions.append(decision)
        if decision["keep"]:
            keep_indexes.add(index)

    if selector.adjacency_window:
        anchor_indexes = {
            decision["index"]
        ...
        for index in anchor_indexes:
            for adjacent in range(
                index - selector.adjacency_window,
                index + selector.adjacency_window + 1,
            ):
...
def write_evidence_selector_outputs(
    chunks: list[TextChunk],
    decisions: list[dict[str, Any]],
